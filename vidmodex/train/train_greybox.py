...
from vidmodex.loss.custom_loss import student_loss
from vidmodex.optim.approximate_gradients import compute_grad_norms, measure_true_grad_norm
...
def train_Wdata(config_args, teacher, student, generator, dataloader, device, optimizer, epoch):
    ...
    for i in range(config_args.epoch_itrs):
        ...
        for j, (data, target) in enumerate(dataloader):
            ...
            s_logit = F.softmax(student(out), dim=1)
        ...
        torch.save(generator, "weights/grey/gen_model_%d.pt" % i)
        torch.save(student, "weights/grey/student_model_%d.pt" % i)
        torch.save(teacher,  "weights/grey/teacher_model_%d.pt" % i)
